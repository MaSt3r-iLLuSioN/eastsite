...
*************************************************************/
	...
	// SSL websites.
	$start_url = "https://eastwaycustomhomes.com/";       
	...
	$skip = array (
            "https://eastwaycustomhomes.com/login",
            "https://eastwaycustomhomes.com/register",
            "https://eastwaycustomhomes.com/page/vendors",
            "https://eastwaycustomhomes.com/uploads",
            "https://eastwaycustomhomes.com/account",
            "https://eastwaycustomhomes.com/admin",
        );
	...
	function Scan ($url) {
		...
		$a1   = $html->find('a');
                foreach($html->find('div[id=seo_freq]') as $e){
                    $freq = $e->plaintext;
                }
		foreach ($a1 as $val) {
			...
			if (substr ($next_url, 0, strlen ($start_url)) == $start_url) {
				...
				if (!$ignore) {
					foreach ($extension as $ext) {
                                            if(substr($next_url, -1) == '/' || strpos($next_url, '?'))
                                            {
	...
	fwrite ($pf, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" .
				 "<?xml-stylesheet type=\"text/xsl\" href=\"https://eastwaycustomhomes.com/sitemap/xml-sitemap.xsl\"?>\n" .
