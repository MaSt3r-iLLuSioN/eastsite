    ...
    (function (Model) {
        ...
        var Room = /** @class */ (function () {
            ...
            function Room(floorplan, corners) {
                ...
                this.exteriorCorners = [];
                /** */
                this.edgePointer = null;
                /** floor plane for intersection testing */
                this.floorPlane = null;
                this.name = '';
                this.biggestWall = null;
                this.smallestWall = null;
                this.sqft = 0;
                this.wallCount = 0;
                this.walls = [];
                /** */
            ...
            Room.prototype.updateExteriorCorners = function () {
                var edge = this.edgePointer;
                while (true) {
            ...
            Room.prototype.getCenter3D = function () {
                var X = [];
                var Y = [];
                this.interiorCorners.forEach(function (corner) {
                    X.push(corner.x);
        ...
        var FloorItem = /** @class */ (function (_super) {
            ...
            FloorItem.prototype.isValidPosition = function (vec3) {
                ...
                if (this.obstructFloorMoves) {
                    var rooms = this.model.floorplan.getRooms();
                    for (var i = 0; i < rooms.length; i++) {
                        if (!BP3D.Core.Utils.polygonOutsidePolygon(corners, rooms[i].exteriorCorners, 0, 0) ||
                            BP3D.Core.Utils.polygonPolygonIntersect(corners, rooms[i].exteriorCorners)) {
                ...
                if (this.obstructFloorMoves) {
                    ...
                    for (var i = 0; i < objects.length; i++) {
                        ...
                        if (BP3D.Core.Utils.polygonInsidePolygon(corners, objects[i].getCorners('x', 'z', null), 0, 0) ||
                            BP3D.Core.Utils.polygonPolygonIntersect(corners, objects[i].getCorners('x', 'z', null))) {
                            //console.log('object is in another object');
                            inObject = true;
                        }
                        if (BP3D.Core.Utils.polygonInsidePolygon(objects[i].getCorners('x', 'z', null), corners, 0, 0) ||
                            BP3D.Core.Utils.polygonPolygonIntersect(objects[i].getCorners('x', 'z', null), corners)) {
                            //console.log('object is in another object');
                            inObject = true;
                        }
                    }
...
(function (BP3D) {
    ...
    (function (Floorplanner) {
        ...
        var FloorplannerView = /** @class */ (function () {
            ...
            FloorplannerView.prototype.calculateSqFt = function (X, Y) {
            ...
            FloorplannerView.prototype.getCenter2D = function (X, Y) {
                var points = [];
                for (var i = 0; i < X.length; i++) {
                    var coord = new THREE.Vector2(0, 0);
                    coord.x = X[i];
            ...
            FloorplannerView.prototype.drawRoom = function (room) {
                var scope = this;
                var xArrgs = BP3D.Core.Utils.map(room.corners, function (corner) {
                    return scope.viewmodel.convertX(corner.x);
                });
                var yArrgs = BP3D.Core.Utils.map(room.corners, function (corner) {
                    return scope.viewmodel.convertY(corner.y);
                });
                this.drawPolygon(xArrgs, yArrgs, true, roomColor);
                //draw extra room info
                var centerRoom = this.getCenter2D(xArrgs, yArrgs);
                var sqFt = this.calculateSqFt(xArrgs, yArrgs);
                room.sqft = Math.floor(sqFt / 235);
                var rooms = room.floorplan.getRooms();
                for (var i = 0; i < rooms.length; i++) {
                    var myRoomUuid = room.getUuid();
                    var otherRoomUuid = rooms[i].getUuid();
                    if (myRoomUuid == otherRoomUuid) {
                        room.name = "Room " + (i + 1);
                        break;
                    }
                }
                ...
                this.context.strokeStyle = "#ffffff";
                this.context.lineWidth = 4;
                this.context.strokeText(room.name, centerRoom.x - (room.name.length / 2), centerRoom.y);
                this.context.fillText(room.name, centerRoom.x - (room.name.length / 2), centerRoom.y);
                var sqString = room.sqft + ' sq';
                this.context.strokeText(sqString, centerRoom.x - (sqString.length / 2), centerRoom.y + 12);
                this.context.fillText(sqString, centerRoom.x - (sqString.length / 2), centerRoom.y + 12);
            };
            /** */
    ...
    (function (Three) {
        Three.Controller = function (three, model, camera, element, controls, hud) {
            ...
            function setGroundPlane() {
                // setup texture
                var floorTexture = THREE.ImageUtils.loadTexture('assets/skyboxs/grasstexture.jpg');
                ...
                var size = 10000;
                plane = new THREE.Mesh(new THREE.PlaneGeometry(size, size), floorMaterialTop);
...
(function (BP3D) {
    var Three;
    (function (Three) {
        Three.Main = function (floorplanner, model, element, canvasElement, opts) {
            var scope = this;
            var options = {
                resize: true,
                pushHref: false,
            ...
            function spin() {
                if (options.spin && !options.overrideSpin && !mouseDown) {
                    var theta = 2 * Math.PI * options.spinSpeed * (Date.now() - lastRender);
                    scope.controls.rotateLeft(theta);
                    scope.controls.update();
                }
            }
            this.setSpin = function (spinning) {
                options.spin = spinning;
            };
            this.getSpin = function () {
                return options.spin;
            };
            this.getWasSpinning = function () {
                return options.wasSpinning;
            };
            this.setWasSpinning = function (spinning) {
                options.wasSpinning = spinning;
            };
            this.setNeedsUpdate = function (update) {
                needsUpdate = update;
            };
            this.setSpinOverride = function (override) {
                options.overrideSpin = override;
            };
            this.getSpinOverride = function () {
                return options.overrideSpin;
            };
...
(function (BP3D) {
    ...
    var Blueprint3d = /** @class */ (function () {
        ...
        function Blueprint3d(options) {
            ...
            this.three = new BP3D.Three.Main(this.floorplanner, this.model, options.threeElement, options.threeCanvasElement, {});
        }
        return Blueprint3d;
    }());
...
console.log("Home Designer " + BP3D.Core.Version.getInformalVersion()
    + " (" + BP3D.Core.Version.getTechnicalVersion() + ") By IndyDevGuy");
